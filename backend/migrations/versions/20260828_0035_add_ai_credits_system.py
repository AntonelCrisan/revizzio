...
# feature: (size_tier, threshold_max, credits) rows, matching the size/cost
# table from the subscription-limits spec (section 6). threshold_max is the
# unit measured per feature: chat = combined message+context character
# count, quiz = total question count, flashcards = card count,
# summary/explanation = source material estimated page count. NULL
# threshold_max marks the top (uncapped) tier.
...
def upgrade() -> None:
    ...
    # The new columns above apply the same conservative (Beginner-tier)
    # server_default to every existing plan row, including paying
    # focus/pro subscribers. Without this backfill, deploying this
    # migration would immediately cut off AI chat access (ai_chat_enabled
    # defaults to false) and drop OCR/page limits for every current
    # focus/pro subscriber until an admin manually re-entered the right
    # numbers through the admin UI. Backfill real per-plan values now so
    # the cutover is immediate and correct.
    op.execute(
        sa.text(
