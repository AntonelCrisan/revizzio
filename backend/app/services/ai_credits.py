...
# Features gated behind the same "AI Chat" plan entitlement (ai_chat_enabled)
# as the interactive chat itself — grouping them together preserves the
# behaviour of the single boolean pro-only gate this system replaces.
AI_CHAT_GATED_FEATURES: frozenset[str] = frozenset({"chat", "explanation"})

# Fallback used only when a user has no `current_plan` row at all (should not
# happen in practice - every user is assigned a plan at registration/webhook
# time - but mirrors the defensive fallback already used for other limits).
...
class AiCreditsService:
    ...
    async def credits_used_this_cycle(
        self, user: User, window_start: datetime, window_end: datetime
    ) -> int:
    ...
    async def ensure_can_consume(
        ...
        *,
        ...
        """Raise if `user` can't afford `feature`/`tier` right now.

        Returns the credit cost on success, so the caller can pass it
        straight to `charge()` after the AI call succeeds.
        """
    ...
    async def ensure_ocr_budget(
        ...
        *,
        ...
        """Raise if OCR-ing `pages_needed` more pages would exceed the plan's
        OCR budget for the current cycle. Checked BEFORE spending a paid OCR
        call, using a local page count rather than Mistral's own count.
        """
        window_start, window_end = window
    ...
    async def charge(
        ...
        *,
        ...
        """Best-effort usage log write.

        A failure here must never turn an already-successful AI call into a
        reported failure for the user, so this commits on its own and
        swallows/logs any error instead of propagating it.
        """
        try:
